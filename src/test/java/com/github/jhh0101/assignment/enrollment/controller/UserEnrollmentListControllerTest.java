...
public class UserEnrollmentListControllerTest {
    ...
    @Test
    @DisplayName("강의별 수강생 목록 조회 성공 테스트")
    void UserEnrollmentList_success() throws Exception {
        Long userId = 1L;
        Long courseId = 1L;
        ...
        given(enrollmentService.userEnrollmentList(eq(userId), eq(courseId), any(Pageable.class)))
                .willReturn(pageResponse);

        mockMvc.perform(get("/api/enrollment/user/list/{courseId}", courseId)
                        .param("userId", "1")
                        .param("page", "0")
                        .param("size", "10")
                ...
                .andExpect(jsonPath("$.message").value("강의별 수강생 목록 조회 성공"))
        ...
        then(enrollmentService).should(times(1)).userEnrollmentList(anyLong(), anyLong(), any(Pageable.class));
    }

    @Test
    @DisplayName("강의별 수강생 목록 조회 성공 테스트 - 수강자가 없는 경우 (빈 페이지)")
    void UserEnrollmentList_success_empty() throws Exception {
        Long userId = 1L;
        Long courseId = 1L;
        ...
        mockMvc.perform(get("/api/enrollment/user/list/{courseId}", 1L)
                        .param("userId", "1")
                        .param("page", "0")
                        .param("size", "10")
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    @DisplayName("강의별 수강생 목록 조회 실패 테스트 - 강의를 찾을 수 없음")
    void UserEnrollmentList_course_not_found() throws Exception {
        willThrow(new CustomException(ErrorCode.COURSE_NOT_FOUND))
                .given(enrollmentService).userEnrollmentList(anyLong(), anyLong(), any(Pageable.class));
    ...
    @Test
    @DisplayName("강의별 수강생 목록 조회 실패 테스트 - 사용자를 찾을 수 없음")
    void UserEnrollmentList_user_not_found() throws Exception {
    ...
    @Test
    @DisplayName("강의별 수강생 목록 조회 실패 테스트 - Creator 권한을 가지고 있지 않음")
    void UserEnrollmentList_user_not_creator() throws Exception {
        willThrow(new CustomException(ErrorCode.USER_NOT_CREATOR))
                .given(enrollmentService). userEnrollmentList(anyLong(), anyLong(), any(Pageable.class));
    ...
    @Test
    @DisplayName("강의별 수강생 목록 조회 실패 테스트 - 담당 강사 정보가 일치하지 않음")
    void UserEnrollmentList_user_not_course_owner() throws Exception {
        willThrow(new CustomException(ErrorCode.NOT_COURSE_OWNER))
                .given(enrollmentService). userEnrollmentList(anyLong(), anyLong(), any(Pageable.class));
